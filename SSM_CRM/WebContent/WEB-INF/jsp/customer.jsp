						...
						<div class="form-group">
							...
							<select	class="form-control" id="customerFrom" placeholder="客户来源" name="custSource">
								...
								<c:forEach items="${fromType}" var="item">
									<c:if test="${item.dict_id == custSource}">
								...
								<c:forEach items="${page.rows}" var="row">
									<tr>
										...
										<td>
											...
											<a href="#" class="btn btn-danger btn-xs" onclick="deleteCustomer(${row.cust_id},'${row.cust_name}')">删除</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<select	class="form-control" id="edit_customerFrom" placeholder="客户来源" name="cust_source">
									...
									<c:forEach items="${fromType}" var="item">
										<c:if test="${item.dict_id == custSource}">
											<option value="${item.dict_id}"  selected>${item.dict_item_name }</option>
										</c:if>
										<c:if test="${item.dict_id != custSource}">
											<option value="${item.dict_id}" >${item.dict_item_name }</option>
										</c:if>
	...
	<script type="text/javascript">
		function editCustomer(id) {
			$.ajax({
				...
				success:function(data) {
					...
					$("#edit_customerName").val(data.cust_name);//此处是"001","002","006",形式
		...
		function updateCustomer() {
			$.post("<%=basePath%>customer/update.action",
				...
				function(data){
					alert("客户信息更新成功！");
					window.location.reload();//无论返回什么都重新加载页面
			});
		}
		
		function deleteCustomer(id,name) {
			
			if(confirm('确实要删除该客户吗?'+id+name)) {
